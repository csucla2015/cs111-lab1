...
//TODO refactor alloc later
  ...
  if(next_char == '&')
  {
	...
	else error(1,0,"%d: Incorrect syntax near token \'&&\'",stream->line_number);
 }
   ...
   if(next_char == '|')
  {
	...
	token_size++;
        next_char = (*stream->get_next_byte)(stream->get_next_byte_argument);
	if(next_char == '|') error(1,0,"%d: Incorrect syntax near token \'||\'",stream->line_number);	
	}
...
command_t parse_simple_command(command_stream_t stream)
{
	...
	//if(next_token_type(stream->current_token) == RIGHT_ROUND)
		//error(1,0,"%d: Unexpected parenthesis found", stream->line_number);	
...
command_t parse_subshell_command(command_stream_t stream)
{
	...
	if(next_token_type(stream->current_token) == NEWLINE)
		skip_newline(stream);
	else{};
	...
	error(1,0,"%d: Unmatched parentheses", stream->line_number);
	return NULL;
...
command_t parse_pipe_subshell(command_stream_t stream)
{
	command_t left = parse_subshell_command(stream);
        command_t right = NULL;
        command_t pipe = NULL;
        while(next_token_type(stream->current_token) == PIPE)
        {
                pipe = make_new_command(PIPE_COMMAND);
                get_next_token(stream);
                right = parse_subshell_command(stream);
                pipe->u.command[0] = left;
                pipe->u.command[1] = right;
                left = pipe;
        }
...
command_t parse_and_or_subshell(command_stream_t stream)
{
	command_t left = parse_pipe_subshell(stream);
        command_t right = NULL;
        while(next_token_type(stream->current_token) == AND || next_token_type(stream->current_token) == OR)
        {
                if(next_token_type(stream->current_token) == AND)
			...
			if(next_token_type(stream->current_token) == NEWLINE) skip_newline(stream);
                        if(next_token_type(stream->current_token) == END_OF_FILE) error(1,0,"%d: Unexpected end of file after token \'&&\'", stream->line_number);
			right = parse_pipe_subshell(stream);
                        and->u.command[0] = left;
                        and->u.command[1] = right;
                        left = and;
                }
...
{
	command_t left = parse_and_or_subshell(stream);
        command_t right = NULL;
        while(next_token_type(stream->current_token) == SEMICOLON)// || next_token_type(stream->current_token) == NEWLINE)
        {
                command_t sequence = make_new_command(SEQUENCE_COMMAND);
...
read_command_stream (command_stream_t s)
{
  ...
  return parse_complete_subshell(s);
